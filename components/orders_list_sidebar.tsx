...
export default function LiveOrdersMain() {
  ...
  useEffect(() => {
    if (!process.env.NEXT_PUBLIC_PUSHER_KEY || !user?.userId) return;
    ...
    channel.bind('order-update', (data: any) => {
      // Only update if it's for this waiter
    ...
    channel.bind('order-ready', (data: any) => {
      // Notification when kitchen marks order as ready
      if (data.waiterId === user?.userId) {
        dispatch(updateOrderLocally(data));
        toast({
          title: 'Order Ready!',
          description: `Order #${data.orderNumber} is ready for pickup`,
        });
      }
    });
          ...
          <div className='space-y-2'>
            ...
            <div className='space-y-1'>
              ...
              {(order.orderLines?.length || 0) > 2 && (
                <p className='text-xs text-gray-500'>
                  +{(order.orderLines?.length || 0) - 2} more items
                </p>
              )}
            </div>
    ...
    <div className='flex flex-col h-full'>
              ...
              <div key={index} className='flex items-center gap-4'>
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <p className='text-sm text-gray-500'>
                    Category: {line.menuItem?.category || 'Food'}
                  </p>
                  {line.notes && (
      ...
      <div className='p-4 lg:p-6 border-t border-gray-200'>
        <div className='space-y-2 mb-4'>
          ...
          <div className='flex justify-between text-sm'>
            <span>
              Tax ({company ? (company.taxRate * 100).toFixed(0) : 10}%):
            </span>
            <span>
              {company?.currency || '$'}
              {tax.toFixed(2)}
        ...
        {selectedOrder ? (
          ...
          <div className='flex flex-col items-center justify-center h-full text-center p-6'>
            ...
            <p className='text-gray-500'>
              Choose an order from the list to view details and manage payment
            </p>
          </div>
        )}
      </div>
      ...
      <Dialog open={updateOrderOpen} onOpenChange={setUpdateOrderOpen}>
        <DialogContent className='max-w-[95vw] lg:max-w-7xl max-h-[95vh] overflow-hidden p-0'>
          ...
          <div className='h-[85vh] overflow-hidden'>
            {selectedOrder && <OrderScreen orderId={selectedOrder.id} />}
          </div>
        </DialogContent>
      </Dialog>
                  ...
                  <div className='space-y-4'>
                    ...
                    <div className='space-y-2'>
                      ...
                      <Input
                        ...
                        value={rounding}
                        onChange={e =>
                          setRounding(Number.parseFloat(e.target.value) || 0)
                        }
                      />
                    </div>
                  </div>
                  ...
                  <div className='bg-gray-50 p-4 lg:p-6 rounded-lg space-y-4'>
                    ...
                    <div className='space-y-3'>
                      ...
                      <div className='flex justify-between text-base'>
                        <span>
                          Tax (
                          {company ? (company.taxRate * 100).toFixed(0) : 10}%):
                        </span>
                        <span className='font-medium'>
                          {company?.currency || '$'}
                          {tax.toFixed(2)}
