...
export default function OrderDetailView({ order, company, Menuitems, onAccept, onComplete }: OrderDetailViewProps) {
  ...
  const calculateTotal = () => {
    ...
    return order.orderLines.reduce((total, line) => total + (line.totalPrice || 0), 0)
  }
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="space-x-2">
          {order.orderStatus === OrderStatus.PENDING && (
            <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={() => onAccept(order)}>
              <ChefHat className="h-4 w-4 mr-2" /> Accept Order
            </Button>
          )}

          {order.orderStatus === OrderStatus.PROCESSING && (
            <Button className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={() => onComplete(order)}>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <div className="flex items-center">
                      ...
                      <h3 className="font-medium text-gray-800 ml-2">{getMenuItemName(line.menuItemId || "")}</h3>
                    </div>
