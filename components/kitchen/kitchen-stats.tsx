...
import { Clock, ChefHat, CheckCircle2, AlertTriangle, Utensils } from "lucide-react"
...
export default function KitchenStats({ orders }: KitchenStatsProps) {
  ...
  const pendingCount = orders.filter((order) => order.orderStatus === OrderStatus.PENDING).length
  const processingCount = orders.filter((order) => order.orderStatus === OrderStatus.PROCESSING).length
  const completedTodayCount = orders.filter((order) => {
    ...
    const isToday = completedDate ? completedDate.setHours(0, 0, 0, 0) === new Date().setHours(0, 0, 0, 0) : false
    return isCompleted && isToday
  }).length

  // Calculate average preparation time
  const completedOrders = orders.filter(
    (order) => order.orderStatus === OrderStatus.COMPLETED && order.createdAt && order.updatedAt,
  )
  ...
  const totalItemsInPreparation = orders
    ...
    .reduce((total, order) => {
      return total + order.orderLines!.reduce((sum, line) => sum + line.quantity, 0)
    }, 0)
    ...
    <Card className={`border ${colorMap[color]}`}>
      <CardContent className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className={`p-2 rounded-full ${iconColorMap[color]}`}>{icon}</div>
        </div>
