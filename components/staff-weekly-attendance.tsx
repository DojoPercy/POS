...
export function StaffWeeklyAttendance({
  ...
  const getStatusColor = (day: WeeklyDay) => {
    ...
    if (day.hasAttendance && day.attendance?.status === 'SIGNED_IN') return 'text-blue-600';
  ...
  const getStatusIcon = (day: WeeklyDay) => {
    ...
    if (day.isCompleted) return <CheckCircle className="h-4 w-4 text-green-600" />;
    if (day.hasAttendance && day.attendance?.status === 'SIGNED_IN') return <Clock className="h-4 w-4 text-blue-600" />;
    if (day.hasShift && !day.hasAttendance) return <XCircle className="h-4 w-4 text-red-600" />;
          ...
          <div className="flex items-center space-x-4">
            <div className="text-center">
              ...
              <p className="text-lg font-bold text-blue-600">
                {weeklyData.summary.completedShifts}/{weeklyData.summary.totalShifts}
              </p>
            </div>
            <div className="text-center">
          ...
          {weeklyData.weeklySchedule.map((day, index) => (
            <div
              ...
              className={`p-3 rounded-lg border ${
                day.hasShift ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200'
              }`}
            >
              <div className="text-center">
                  ...
                  <div className="space-y-1">
                    ...
                    {day.attendance && (
                      <div className="mt-2 pt-2 border-t border-gray-200">
                        <p className="text-xs text-gray-600">
                          {day.attendance.signInTime && formatTime(day.attendance.signInTime)}
                          {day.attendance.signOutTime && ` - ${formatTime(day.attendance.signOutTime)}`}
                        </p>
          ...
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-blue-600">
              {weeklyData.summary.totalShifts > 0 
                ? Math.round((weeklyData.summary.completedShifts / weeklyData.summary.totalShifts) * 100)
                : 0}%
            </p>
          </div>
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-green-600">
              {weeklyData.summary.completedShifts > 0
                ? formatDuration(weeklyData.summary.totalHours / weeklyData.summary.completedShifts)
                : '0h 0m'}
            </p>
          </div>
          <div className="text-center">
            ...
            <Badge 
              variant={weeklyData.summary.completedShifts === weeklyData.summary.totalShifts ? 'default' : 'secondary'}
              className="mt-1"
            >
              {weeklyData.summary.completedShifts === weeklyData.summary.totalShifts ? 'Complete' : 'In Progress'}
            </Badge>
          </div>
        </div>
      </CardContent>
