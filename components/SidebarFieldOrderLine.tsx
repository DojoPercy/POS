...
export default function SidebarFieldOrderLine({
  data,
  label,
  columnName,
  iconName,
    ...
    <div className='mt-8 w-full rounded-sm bg-zinc-200 text-zinc-500 py-2 pointer-events-none'>
        ...
        data[columnName ? columnName : label].map((row: any, i: number) => (
          ...
          >
            ...
            <div className='ml-auto my-auto justify-end flex w-1/2'>
              ...
              <span className='my-auto w-1/3 text-right border-l-2 border-l-zinc-300 pr-2'>
                {Intl.NumberFormat('id').format(row.sellUnitPrice)}
              </span>
      ...
      <div className='flex flex-col w-full h-24 px-3.5'>
        <div className='flex w-full h-8'>
          <span className='mr-auto my-auto'>Discount:</span>
          <span className='ml-auto my-auto'>
            {!loading && Object.keys(data).length !== 0
              ? data.discount
                ? Intl.NumberFormat('id').format(-1 * data.discount)
                : '0'
              : '0'}
          </span>
        </div>
        <div className='flex w-full h-8'>
          <span className='mr-auto my-auto'>Rounding:</span>
          <span className='ml-auto my-auto'>
            {!loading && Object.keys(data).length !== 0
              ? data.rounding
                ? Intl.NumberFormat('id').format(data.rounding)
                : '0'
              : '0'}
          </span>
        </div>
