...
export function OrderCard({ order, onComplete }: { order: any, onComplete: (orderId: string) => void }) {
  return (
    <Card className="h-full flex flex-col">
      <CardHeader>
        <CardTitle className="flex justify-between items-center">
          <span>Order #{order.id}</span>
          <Badge variant="outline">{formatDistanceToNow(new Date(order.requiredDate), { addSuffix: true })}</Badge>
        </CardTitle>
      </CardHeader>
      ...
      <CardFooter className="flex justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Waiter ID: {order.waiterId}</p>
        </div>
