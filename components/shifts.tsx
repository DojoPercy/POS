...
export default function ShiftScheduler() {
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setShifts((prev) => prev.map((shift) => (shift.id === draggableId ? updatedShift : shift)))
      }
    } catch (error) {
  ...
  const filteredShifts = shifts.filter((shift) => {
    ...
    const matchesRole = selectedRole === "all" || shift.role.toLowerCase() === selectedRole.toLowerCase()
        ...
        >
          <CardContent className="p-4">
            ...
            <div className="space-y-2">
              ...
              {shift.notes && <p className="text-xs text-gray-600 bg-gray-50 p-2 rounded">{shift.notes}</p>}
            </div>
          </CardContent>
        </Card>
      )}
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">Manage staff schedules across all locations</p>
        </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setNewShift((prev) => ({ ...prev, userId: value }))}
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setNewShift((prev) => ({ ...prev, branchId: value }))}
                  >
                    <SelectTrigger>
      ...
      <DragDropContext onDragEnd={handleDragEnd}>
        <div className="flex gap-6 overflow-x-auto pb-4">
          <StatusColumn status="SCHEDULED" title="Scheduled" shifts={groupedShifts.SCHEDULED} />
          <StatusColumn status="IN_PROGRESS" title="In Progress" shifts={groupedShifts.IN_PROGRESS} />
          <StatusColumn status="COMPLETED" title="Completed" shifts={groupedShifts.COMPLETED} />
          <StatusColumn status="CANCELLED" title="Cancelled" shifts={groupedShifts.CANCELLED} />
          <StatusColumn status="NO_SHOW" title="No Show" shifts={groupedShifts.NO_SHOW} />
        </div>
      </DragDropContext>
                ...
                <div key={day.toISOString()} className="space-y-2">
                  ...
                  <div className="space-y-1 min-h-[100px]">
                    {dayShifts.slice(0, 3).map((shift) => (
                      <div key={shift.id} className={`p-2 rounded text-xs border ${statusColors[shift.status]}`}>
                        <p className="font-medium truncate">{shift.user.fullname}</p>
                    ...
                    {dayShifts.length > 3 && (
                      <div className="text-xs text-gray-500 text-center py-1">+{dayShifts.length - 3} more</div>
                    )}
                  </div>
                </div>
              )
