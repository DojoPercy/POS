...
export default function AddMenuItemForm({ companyId, onAddItem }: { companyId: string; onAddItem: () => void }) {
  ...
  const updateIngredientAmount = (id: string, amount: number) => {
    ...
    setFormData((prev) => ({
      ...prev,
      ingredients: prev.ingredients.map((i) => (i.id === id ? { ...i, amount } : i)),
    }))
  }
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto shadow-lg border-muted">
      <CardHeader className="bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
        ...
        <CardDescription className="text-center">Create a new item for your menu</CardDescription>
      </CardHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Tabs defaultValue="basic" className="w-full">
            <TabsList className="grid grid-cols-2 mb-4">
              ...
              <TabsTrigger value="ingredients" disabled={!formData.useIngredients}>
                Ingredients
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="basic" className="space-y-4">
              ...
              <Accordion type="single" collapsible defaultValue="prices" className="w-full">
                <AccordionItem value="prices">
                  <AccordionTrigger className="text-base font-medium">Price Options</AccordionTrigger>
                  <AccordionContent>
                      ...
                      {formData.prices.map((price, index) => (
                        ...
                        >
                          <div className="flex-1">
                            <Label htmlFor={`price-name-${index}`} className="text-xs mb-1 block">
                              Option Name
                            </Label>
                            <Input
                              id={`price-name-${index}`}
                          ...
                          <div className="flex-1">
                            <Label htmlFor={`price-value-${index}`} className="text-xs mb-1 block">
                              Price
                            </Label>
                            <Input
              ...
              <div className="flex items-center space-x-2 pt-2">
                <Switch
                  ...
                  onCheckedChange={(checked) => handleChange("useIngredients", checked)}
                />
                        ...
                        <ScrollArea className="h-[300px] pr-4">
                          {loadingIngredients ? (
                            ...
                            <div className="text-center py-8 text-muted-foreground">
                              {searchQuery ? "No matching ingredients found" : "No ingredients available"}
                            </div>
                          ) : (
                            <div className="space-y-2">
                              {filteredIngredients.map((ingredient) => (
                                ...
                                >
                                  <div>
                                    ...
                                    <div className="text-xs text-muted-foreground">Unit: {ingredient.unit}</div>
                                  </div>
                                  <Button
                                    type="button"
                                    variant="ghost"
                                    ...
                                    disabled={formData.ingredients.some((i) => i.id === ingredient.id)}
                                  >
                                    {formData.ingredients.some((i) => i.id === ingredient.id) ? "Added" : "Add"}
                                  </Button>
                                </div>
                              ))}
                            </div>
                      ...
                      <CardContent>
                        <ScrollArea className="h-[300px] pr-4">
                          {formData.ingredients.length === 0 ? (
                            <div className="text-center py-8 text-muted-foreground">No ingredients selected</div>
                          ) : (
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Button
                                      ...
                                      size="icon"
                                      className="h-8 w-8"
                                      onClick={() => updateIngredientAmount(ingredient.id, ingredient.amount - 1)}
