...
export function NotificationTest({ companyId }: NotificationTestProps) {
  ...
  const handleTestNotification = async (type: string, testFunction: () => Promise<void>) => {
    setLoading(type)
    try {
      await testFunction()
      toast({
  ...
  const testFunctions = {
    "top-branches": () => AutomaticNotificationService.checkTopBranches(companyId),
    "highest-orders": () => AutomaticNotificationService.checkHighestOrders(companyId),
    "top-attendance": () => AutomaticNotificationService.checkTopAttendance(companyId),
    "low-stock": () => AutomaticNotificationService.checkLowStockAlerts(companyId),
    "sales-milestones": () => AutomaticNotificationService.checkSalesMilestones(companyId),
    "daily-summary": () => AutomaticNotificationService.generateDailySummary(companyId),
    "weekly-achievements": () => AutomaticNotificationService.generateWeeklyAchievements(companyId),
    ...
    <Card>
      ...
      <CardContent>
                    ...
                    <Button
                      ...
                      onClick={() => 
                        handleTestNotification(
                          notification.type, 
                          testFunctions[notification.type as keyof typeof testFunctions]
                        )
                      }
                      disabled={isLoading}
                    >
        ...
        <div className="mt-4 text-xs text-gray-500">
          <p>💡 These tests will create real notifications using your actual data from the API.</p>
