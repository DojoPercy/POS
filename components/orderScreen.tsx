...
export default function OrderScreen({ orderId }: OrderScreenProp) {
  ...
  return (
    <div className='flex flex-col lg:flex-row gap-6 bg-gray-50 h-full'>
      ...
      {/* Order summary */}
      <div
        className={`lg:w-96 lg:block ${isCartOpen ? 'block fixed inset-0 z-50 bg-black/50' : 'hidden'}`}
      >
        <div
          className={`h-full ${isCartOpen ? 'bg-white w-full max-w-md ml-auto' : ''}`}
        >
          <OrderSummary
            cart={cart}
      ...
      <div className='lg:hidden fixed bottom-4 right-4 z-10'>
        ...
        >
          ...
          {totalCartItems > 0 && (
            <Badge className='absolute -top-2 -right-2 px-2 py-1 min-w-[24px]'>
              {totalCartItems}
            </Badge>
          )}
        </Button>
