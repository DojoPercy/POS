    ...
    try {
      ...
      const response = await fetch('/api/shift', {
        ...
        body: JSON.stringify({
          ...
          branchId:
            selectedBranch === 'all' ? template.branchId : selectedBranch,
          companyId,
            ...
            {selectedCell && (
              <div className='p-3 bg-blue-50 rounded-lg'>
                <p className='text-sm text-blue-800'>
                  ...
                  {getTemplateById(selectedCell.templateId)?.name}
                </p>
                <p className='text-sm text-blue-800'>
                  <strong>Day:</strong>{' '}
                  {
                    DAYS_OF_WEEK.find(d => d.value === selectedCell.dayOfWeek)
                      ?.label
                  }
                </p>
                <p className='text-sm text-blue-800'>
                  <strong>Time:</strong> {selectedCell.startTime} -{' '}
