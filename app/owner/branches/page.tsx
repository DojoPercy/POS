...
import { Edit, Trash2, Plus, Search, MapPin, Users, Star, Heart, MoreVertical, Building2, Filter, Grid, List, AlertCircle, CheckCircle } from "lucide-react"
...
const MobileBranchCard = ({ branch, onEdit, onDelete }: { 
  branch: Branch; 
  ...
  <Card className="overflow-hidden">
    <div className="relative">
      <div className="aspect-video bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center"> <img
...
const DesktopBranchCard = ({ branch, onEdit, onDelete }: { 
  branch: Branch; 
  onEdit: (branch: Branch) => void; 
  onDelete: (id: string) => void; 
}) => (
...
export default function BranchManagement() {
  ...
  const fetchBranches = useCallback(async (companyId?: string) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || err.message || "Failed to fetch branches"
  ...
  const handleDelete = useCallback(async (id: string) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || "Failed to delete branch"
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  const handleBranchCreated = useCallback((newBranch: Branch) => {
  ...
  const handleBranchUpdated = useCallback((updatedBranch: Branch) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-slate-900">Branch Management</h1>
          <p className="text-slate-600 mt-1">Manage your restaurant branches and locations</p>
        </div>
