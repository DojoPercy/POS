...
const TrendIndicator = ({ change, size = "sm" }: { change: number; size?: "sm" | "lg" }) => {
...
export default function ImprovedDashboard() {
    ...
    const fetchTodayStats = async () => {
      ...
      try {
        ...
        const yesterdayRevenue =
          yesterdayBranchSummaries?.reduce((sum, branch) => sum + Number.parseFloat(branch.revenue), 0) || 0
        const yesterdayOrders = yesterdayBranchSummaries?.reduce((sum, branch) => sum + branch.sales, 0) || 0
        ...
        const expensesChange =
          yesterdayExpenses > 0 ? ((todayExpenses - yesterdayExpenses) / yesterdayExpenses) * 100 : 0
        ...
        setTodayStats({
          revenue: { current: todayRevenue, previous: yesterdayRevenue, change: revenueChange },
          orders: { current: todayOrders, previous: yesterdayOrders, change: ordersChange },
          avgOrderValue: { current: todayAvgOrderValue, previous: yesterdayAvgOrderValue, change: avgOrderValueChange },
          profit: { current: todayProfit, previous: yesterdayProfit, change: profitChange },
          expenses: { current: todayExpenses, previous: yesterdayExpenses, change: expensesChange },
        })
        ...
        for (let i = 6; i >= 0; i--) {
          ...
          sparklinePromises.push(getSalesSummaryOfBranches(date, date, selectedCompany.id))
        }

        const sparklineResults = await Promise.all(sparklinePromises)
        const revenueSparkline = sparklineResults.map(
          (result) => result?.reduce((sum, branch) => sum + Number.parseFloat(branch.revenue), 0) || 0,
        )
  ...
  const generateKeyInsights = () => {
    ...
    const topBranch = branches.reduce(
      (max, branch) => ((branch.todayRevenue || 0) > (max.todayRevenue || 0) ? branch : max),
      branches[0],
    )
  ...
  const MobileHeader = () => (
    <div className="bg-white border-b border-slate-200 px-4 py-3 lg:hidden">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleRefresh} disabled={refresh}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900">{selectedCompany?.name || "Loading..."}</h1>
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="p-4 lg:p-6">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {StatisticHeaders.slice(0, 3).map((header, i) => (
                      <Button
                        key={header.name}
                        variant={selectedHeader === header ? "default" : "outline"}
                        size="sm"
                ...
                <CardContent>
                  <div className="space-y-3">
                    {activeBranches.slice(0, 3).map((branch, index) => {
                      const isTop = index === 0
                      const isBottom = index === activeBranches.length - 1 && activeBranches.length > 1
                          ...
                          <div className="flex-1 min-w-0">
                            <p
                              className={`font-medium truncate ${
                                isTop ? "text-green-900" : isBottom ? "text-red-900" : "text-slate-900"
                              }`}
                            >
                              {branch.name}
                            </p>
                            <p
                              className={`text-sm truncate ${
                                isTop ? "text-green-700" : isBottom ? "text-red-700" : "text-slate-700"
                              }`}
                            >
                              {selectedCompany?.currency}
                              {(branch.todayRevenue || 0).toLocaleString()} revenue
                            </p>
                          </div>
                          <Badge
                            className={
                ...
                <CardContent>
                  <div className="space-y-3">
                    {keyInsights.map((insight, index) => (
                      <div key={index} className={`flex items-start gap-3 p-3 ${insight.bgColor} rounded-lg`}>
                        {insight.icon}
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-slate-900 truncate">{insight.title}</p>
                          <p className="text-sm text-slate-700 truncate">{insight.description}</p>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="space-y-4 lg:space-y-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xl lg:text-2xl font-bold text-slate-900">Branch Management</h2>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                ...
                >
                  ...
                  <span className="hidden sm:inline">{showInactiveBranches ? "Hide Inactive" : "Show All"}</span>
                </Button>
                  ...
                  <Card key={branch.id} className={branch.status === "inactive" ? "opacity-60" : ""}>
                    <CardContent className="p-4 lg:p-6">
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="font-semibold text-slate-900 truncate flex-1">{branch.name}</h4>
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">
                              {selectedCompany?.currency}
                              {(branch.todayRevenue || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            </span>
                          </div>
                          <div className="flex justify-between text-sm">
        ...
        {activeTab === "financials" && (
          <div className="space-y-4 lg:space-y-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <h2 className="text-xl lg:text-2xl font-bold text-slate-900">Financial Overview</h2>
              <div className="flex flex-col sm:flex-row gap-2">
                <Button variant="outline" size="sm">P&L Report</Button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
              {selectedCompany && (
                <ProfitSummaries companyId={selectedCompany.id} currency={selectedCompany?.currency} />
              )}
        ...
        {activeTab === "menu" && (
          <div className="space-y-4 lg:space-y-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <h2 className="text-xl lg:text-2xl font-bold text-slate-900">Menu Performance</h2>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-center py-12">
                    ...
                    <p className="text-slate-500">Additional menu analytics coming soon</p>
                  </div>
                </CardContent>
              </Card>
            </div>
