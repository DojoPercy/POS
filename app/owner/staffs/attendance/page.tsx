      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-slate-900 truncate">{record.user.fullname}</h3>
            <p className="text-sm text-slate-600 truncate">{record.user.role}</p>
          </div>
        </div>
      ...
      <div className="space-y-2 text-sm">
        ...
        {record.signInTime && (
          <div className="flex items-center justify-between text-xs text-slate-500">
            ...
            {record.signOutTime && <span>Out: {formatTime(record.signOutTime)}</span>}
          </div>
        )}
      </div>
    </CardContent>
    ...
    <CardContent className="p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h3 className="font-semibold text-slate-900">{record.user.fullname}</h3>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
        ...
        {record.signInTime && (
          <div className="flex items-center text-slate-600">
            ...
            <span>{formatTime(record.signInTime)} - {record.signOutTime ? formatTime(record.signOutTime) : "Active"}</span>
          </div>
        )}
...
export default function AttendanceManagement() {
  ...
  const exportData = useCallback(() => {
    ...
    const csvContent = [
      ["Date", "Employee", "Role", "Branch", "Sign In", "Sign Out", "Total Hours", "Status"].join(","),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-slate-900">Attendance Management</h1>
          <p className="text-slate-600 mt-1">Monitor and manage staff attendance across all branches</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-900">{attendanceData.summary.totalRecords}</p>
                </div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-slate-600">Completed Shifts</p>
                  <p className="text-2xl font-bold text-green-600">{attendanceData.summary.completedShifts}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-600">{attendanceData.summary.totalHours.toFixed(1)}h</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-orange-600">{attendanceData.summary.averageHours.toFixed(1)}h</p>
                </div>
                ...
                {attendanceData ? (
                  <div className="space-y-4">
                    {attendanceData.branchStats.map((stat) => (
                      <div key={stat.branchId} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                        <div>
                          <p className="font-medium text-slate-900">{stat.branchName}</p>
                          <p className="text-sm text-slate-600">{stat.branchCity}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold text-slate-900">{stat.totalShifts} shifts</p>
                          <p className="text-sm text-slate-600">{stat.averageHours.toFixed(1)}h avg</p>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-slate-900">
                Attendance Records
                <span className="text-slate-500 ml-2">({filteredRecords.length})</span>
              </h2>
            </div>
                ...
                {attendanceData ? (
                  <div className="space-y-4">
                    {attendanceData.userStats.slice(0, 5).map((stat) => (
                      <div key={stat.userId} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="font-medium text-slate-900">{stat.userName}</p>
                            <p className="text-sm text-slate-600">{stat.userRole}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold text-slate-900">{stat.totalShifts} shifts</p>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-semibold text-slate-900">
                        {((attendanceData.summary.completedShifts / attendanceData.summary.totalRecords) * 100).toFixed(1)}%
                      </span>
                    </div>
                    <Progress 
                      value={(attendanceData.summary.completedShifts / attendanceData.summary.totalRecords) * 100} 
                      className="h-2" 
                    />
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-4">
                      ...
                      <div className="text-center p-3 bg-red-50 rounded-lg">
                        <p className="text-2xl font-bold text-red-600">{attendanceData.summary.incompleteShifts}</p>
