...
export default function EnhancedShiftTimetable() {
  ...
  const fetchData = async (companyId: string) => {
    try {
      setLoading(true)
      const [usersResponse, branchesResponse, shiftsResponse, templatesResponse] = await Promise.all([
        ...
        fetch(`/api/shift-templates?companyId=${companyId}&branchId=${selectedBranch}`),
      ])

      const responses = [usersResponse, branchesResponse, shiftsResponse, templatesResponse]
      responses.forEach((res, i) => {
        if (!res.ok) throw new Error(`Request ${i + 1} failed with status ${res.status}`)
      })

      const [usersData, branchesData, shiftsData, templatesData] = await Promise.all([
        usersResponse.json(),
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    const timeSlotData = TIME_SLOTS.find((slot) => slot.start === startTime && slot.end === endTime)
  ...
  const getShiftsForSlot = (dayOfWeek: number, startTime: string, endTime: string) => {
    return shifts.filter(
      (shift) => shift.dayOfWeek === dayOfWeek && shift.startTime === startTime && shift.endTime === endTime,
    )
  }

  const getTemplateForSlot = (dayOfWeek: number, startTime: string, endTime: string) => {
    return templates.find(
      (template) =>
        template.dayOfWeek === dayOfWeek && template.startTime === startTime && template.endTime === endTime,
    )
  }
      ...
      {(provided, snapshot) => (
        <div
          ...
          className={`mb-3 cursor-move transition-all duration-300 ${
            snapshot.isDragging ? "shadow-2xl rotate-2 scale-110 z-50" : "hover:shadow-lg hover:scale-[1.02]"
          }`}
        >
          <div className="bg-white rounded-xl border border-gray-200 p-4 hover:border-gray-300 shadow-sm hover:shadow-md transition-all duration-200">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-sm text-gray-900 truncate mb-1">{user.fullname}</p>
                <Badge
                  variant="outline"
                  className={`text-xs font-medium border-0 shadow-sm ${
                    roleColors[user.role.toLowerCase() as keyof typeof roleColors] || roleColors.default
                  }`}
                >
                  {user.role}
                </Badge>
  ...
  const TimeSlot = ({ day, timeSlot }: { day: (typeof DAYS_OF_WEEK)[0]; timeSlot: (typeof TIME_SLOTS)[0] }) => {
    const slotShifts = getShiftsForSlot(day.value, timeSlot.start, timeSlot.end)
    const template = getTemplateForSlot(day.value, timeSlot.start, timeSlot.end)
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center space-x-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-semibold text-gray-900 truncate">{shift.user.fullname}</p>
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            >
                              <div className="flex items-center gap-1.5">
                                {stateConfig.icon}
                                <span className="hidden sm:inline">{stateConfig.label}</span>
                              </div>
                            </Badge>
                          </div>
                        </div>
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" size="icon" className="h-7 w-7 flex-shrink-0 hover:bg-white/50">
                            <MoreVertical className="h-3.5 w-3.5" />
                          </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="bg-white border shadow-xl rounded-xl">
                          <DropdownMenuItem onClick={() => handleShiftStateChange(shift.id, "ACTIVE")}>
                            <Play className="h-3 w-3 mr-2 text-blue-600" />
                            Set Active
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleShiftStateChange(shift.id, "ASSIST")}>
                            <Star className="h-3 w-3 mr-2 text-yellow-600" />
                            Set Assist
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleShiftStateChange(shift.id, "BREAK")}>
                            <Coffee className="h-3 w-3 mr-2 text-orange-600" />
                            Set Break
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleShiftStateChange(shift.id, "COMPLETED")}>
                            <CheckCircle className="h-3 w-3 mr-2 text-green-600" />
                            Mark Complete
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleShiftStateChange(shift.id, "INACTIVE")}>
                          ...
                          <DropdownMenuItem className="text-red-600" onClick={() => handleDeleteShift(shift.id)}>
            ...
            {slotShifts.length === 0 && !snapshot.isDraggingOver && (
              <div className="flex items-center justify-center h-full text-gray-400 min-h-[100px]">
                <div className="text-center">
                  ...
                  <p className="text-sm font-medium opacity-60">Drop staff here</p>
  ...
  const navigateDay = (direction: "prev" | "next") => {
    ...
    if (direction === "prev") {
      ...
      const nextIndex = currentIndex === DAYS_OF_WEEK.length - 1 ? 0 : currentIndex + 1
    ...
    <div className="p-4 sm:p-6 space-y-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      ...
      <div className="flex flex-col space-y-4">
        <div className="text-center lg:text-left">
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-2">Staff Scheduling</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center justify-center lg:justify-start">
          ...
          <Dialog open={isTemplateDialogOpen} onOpenChange={setIsTemplateDialogOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" className="bg-white shadow-sm border-gray-200 rounded-xl hover:shadow-md">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setNewTemplate((prev) => ({ ...prev, branchId: value }))}
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setNewTemplate((prev) => ({ ...prev, dayOfWeek: value }))}
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewTemplate((prev) => ({ ...prev, maxStaff: e.target.value }))}
                  />
                </div>
  ...
  {showStaffPanel && (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-40 lg:hidden" onClick={() => setShowStaffPanel(false)}>
      <div className="fixed bottom-0 left-0 right-0 bg-white rounded-t-xl p-4 max-h-[70vh] overflow-hidden">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button variant="ghost" size="sm" onClick={() => setShowStaffPanel(false)}>
            ✕
          </Button>
        </div>
          ...
          <div className="overflow-x-auto">
            ...
            <div className="grid grid-cols-8 gap-4 min-w-[700px] lg:min-w-0">
              ...
              {DAYS_OF_WEEK.map((day) => (
                ...
                >
                  <div>{day.short}</div>
                  <div className="text-xs text-gray-500 mt-1 font-normal">{day.label}</div>
                </div>
              ))}
            </div>
            ...
            {TIME_SLOTS.map((timeSlot, index) => (
              <div key={`${timeSlot.start}-${timeSlot.end}`} className="grid grid-cols-8 gap-4 mb-6 min-w-[700px] lg:min-w-0">
                <div className="p-4 bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl border shadow-sm">
                  <div className="text-sm font-bold text-gray-900 mb-1">{timeSlot.label}</div>
                  <div className="text-xs text-gray-600 font-medium">
                    {timeSlot.start} - {timeSlot.end}
                  </div>
                  <div className="text-xs text-gray-400 mt-2">Slot {index + 1}</div>
                </div>
                {DAYS_OF_WEEK.map((day) => (
                  <TimeSlot key={`${day.value}-${timeSlot.start}`} day={day} timeSlot={timeSlot} />
                ))}
              </div>
            ))}
          </div>
    ...
    <Card className="bg-white">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-lg">
          ...
          {DAYS_OF_WEEK.find((day) => day.value === selectedDay)?.label} Schedule
        </CardTitle>
      </CardHeader>
      <CardContent className="p-4">
        <div className="space-y-4">
          {TIME_SLOTS.map((timeSlot) => (
            <div key={`${timeSlot.start}-${timeSlot.end}`} className="space-y-3">
              ...
              <TimeSlot day={DAYS_OF_WEEK.find((day) => day.value === selectedDay)!} timeSlot={timeSlot} />
            </div>
          ))}
        </div>
      </CardContent>
        ...
        <CardContent>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="text-center p-3 sm:p-4 bg-blue-50 rounded-lg">
              <div className="text-xl sm:text-2xl font-bold text-blue-600">{shifts.length}</div>
              <div className="text-xs sm:text-sm text-gray-600">Total Shifts</div>
            </div>
            <div className="text-center p-3 sm:p-4 bg-green-50 rounded-lg">
              <div className="text-xl sm:text-2xl font-bold text-green-600">{templates.length}</div>
              <div className="text-xs sm:text-sm text-gray-600">Templates</div>
            </div>
            <div className="text-center p-3 sm:p-4 bg-purple-50 rounded-lg">
              <div className="text-xl sm:text-2xl font-bold text-purple-600">{filteredUsers.length}</div>
