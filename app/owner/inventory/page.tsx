...
import { RefreshCw, Clipboard, Download, Package, AlertTriangle } from "lucide-react"
...
export default function InventoryPage() {
  ...
  const companyId = useSelector((state: any) => state.company?.currentCompanyId)
 ...
 const fetchBranches = useCallback(async () => {
  ...
  const fetchInventoryStock = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/inventory_stock?branchId=${selectedBranch}`)
  ...
  const getFilteredAndSortedData = (): InventoryItem[] => {
    ...
    const filtered = filterValue
      ? data.filter((item: any) => item.name.toLowerCase().includes(filterValue.toLowerCase()))
      : data
  ...
  const exportToCSV = () => {
    ...
    const csvData = inventoryData.map((item: any) => [item.name, item.unit, item.stock])
  ...
  const copyToClipboard = () => {
    const headers = ["Ingredient Name", "Unit", "Current Stock"]
    const textData = inventoryData.map((item: any) => [item.name, item.unit, item.stock])
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground">Track and manage ingredient stock across all branches</p>
        </div>
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>Select a branch to manage inventory</CardDescription>
            </CardHeader>
            <CardContent>
              {isLoadingBranches ? (
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
              <div>
                ...
                <CardDescription>
                  ...
                  {branches.find((branch: any) => branch.id === selectedBranch)?.name || "selected branch"}
                </CardDescription>
              </div>
              <InventoryFilters
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value="table" className="space-y-4">
                {isLoading ? (
                  ...
                  <InventoryTable inventoryData={inventoryData} onUpdateStock={handleUpdateStock} />
                )}
              </TabsContent>
              ...
              <TabsContent value="grid">
                {isLoading ? (
                  ...
                  <InventoryGrid inventoryData={inventoryData} onUpdateStock={handleUpdateStock} />
                )}
              </TabsContent>
            </Tabs>
          </CardContent>
