...
import { Search, Plus, Grid, List, Edit, Trash2, MoreVertical, ChefHat, Filter, Star, Clock, AlertCircle } from "lucide-react"
...
const MobileMenuItemCard = ({ item, onEdit, onDelete }: {
  item: MenuItem
...
const DesktopMenuItemCard = ({ item, onEdit, onDelete }: {
  item: MenuItem
  onEdit: (item: MenuItem) => void
  onDelete: (id: string) => void
}) => {
...
export default function MenuManagement() {
  ...
  const filteredAndSortedItems = useMemo(() => {
    let filtered = menuItems.filter((item: MenuItem) => {
      const matchesSearch =
  ...
  const groupedItems = useMemo(() => {
    return filteredAndSortedItems.reduce((acc: Record<string, MenuItem[]>, item: MenuItem) => {
  ...
  const handleItemCreated = useCallback((newItem: MenuItem) => {
  ...
  const handleItemUpdated = useCallback((updatedItem: MenuItem) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Manage your restaurant menu items and categories</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  ${(menuItems.reduce((sum: any, item: { price: { price: any }[] }) => sum + (item.price?.[0]?.price || 0), 0) / Math.max(menuItems.length, 1)).toFixed(2)}
                </p>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-600">With Images</p>
                <p className="text-2xl font-bold text-purple-600">
                  {menuItems.filter((item: { imageUrl: any }) => item.imageUrl).length}
                </p>
              </div>
            ...
            <Card key={categoryName}>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {categoryName}
                  <Badge variant="outline">{(items as MenuItem[]).length} items</Badge>
                </CardTitle>
              </CardHeader>
              <CardContent>
