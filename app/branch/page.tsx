...
export default function ManagerProfile() {
  ...
  const breadcrumbs = [{ label: "Dashboard", href: "/branch" }, { label: "Profile" }]

   const fetchBranchSummary = useCallback(async (targetBranchId: string, fromDate: Date, toDate: Date) => {
    try {
      setSummaryLoading(true)
      const from = format(fromDate, "yyyy-MM-dd")
      const to = format(toDate, "yyyy-MM-dd")

      const response = await fetch(`/api/orders/branch?branchId=${targetBranchId}&from=${from}&to=${to}`)
  ...
  const handleRefresh = () => {
    if ((branchId || user?.branchId) && dateRange?.from && dateRange?.to) {
      fetchBranchSummary(branchId?.toString() || user?.branchId || "", dateRange.from, dateRange.to)
    }
  }
  ...
  useEffect(() => {
    const fetchUserAndBranch = async () => {
      try {
        ...
        const branchDetails = await getBranchById((branchId?.toString() || decodedToken.branchId) ?? "")
        ...
        if (dateRange?.from && dateRange?.to) {
          fetchBranchSummary((branchId?.toString() || decodedToken.branchId) ?? "", dateRange.from, dateRange.to)
        }
      } catch (error) {
        console.error(error)
        toast({
  ...
  const waiterPerformanceData = branchSummary ? Object.values(branchSummary.byWaiter) : []
  const topWaiters = waiterPerformanceData.sort((a, b) => b.amount - a.amount).slice(0, 5)

  const branchStats = [
    {
      title: "Total Revenue",
      value: summaryLoading ? "..." : `$${branchSummary?.totalAmount.toFixed(2) || "0.00"}`,
      icon: DollarSign,
    ...
    {
      title: "Total Orders",
      value: summaryLoading ? "..." : branchSummary?.totalOrders.toString() || "0",
      icon: ShoppingBag,
      color: "text-blue-600",
      bgColor: "bg-blue-50",
      change: "+12%",
    ...
    {
      title: "Items Served",
      value: summaryLoading ? "..." : branchSummary?.totalItems.toString() || "0",
      icon: Target,
    ...
    },
    {
      ...
      value: summaryLoading ? "..." : `$${branchSummary?.averageOrderValue.toFixed(2) || "0.00"}`,
      icon: TrendingUp,
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg">
                    ...
                    {branchSummary?.lastUpdated && (
                      <p className="text-xs text-gray-500 mt-1">
                        Last updated: {format(new Date(branchSummary.lastUpdated), "MMM dd, yyyy 'at' HH:mm")}
                      </p>
                    )}
                  </div>
                </div>
                    ...
                    <CardContent className="space-y-4">
                      {branchSummary?.topPerformers.topWaiter && (
                        <div className="flex items-center gap-3 p-3 bg-yellow-50 rounded-lg">
                          ...
                          <div>
                            <p className="font-medium text-gray-900">Top Waiter</p>
                            <p className="text-sm text-gray-600">
                              {branchSummary.topPerformers.topWaiter.waiterName || "Unknown"} - $
                              {branchSummary.topPerformers.topWaiter.amount.toFixed(2)}
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      {branchSummary?.topPerformers.busiestDay && (
                        <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
                          ...
                          <div>
                            <p className="font-medium text-gray-900">Busiest Day</p>
                            <p className="text-sm text-gray-600">
                              {format(new Date(branchSummary.topPerformers.busiestDay.date), "MMM dd, yyyy")} -{" "}
                              {branchSummary.topPerformers.busiestDay.orders} orders
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      {branchSummary?.topPerformers.highestSalesDay && (
                        <div className="flex items-center gap-3 p-3 bg-green-50 rounded-lg">
                          ...
                          <div>
                            <p className="font-medium text-gray-900">Highest Sales Day</p>
                            <p className="text-sm text-gray-600">
                              {format(new Date(branchSummary.topPerformers.highestSalesDay.date), "MMM dd, yyyy")} - $
                              {branchSummary.topPerformers.highestSalesDay.amount.toFixed(2)}
                            </p>
                          </div>
                        </div>
                      )}
                    ...
                    {summaryLoading ? (
                      ...
                      <div className="space-y-4">
                        {topWaiters.map((waiter, index) => (
                          <div key={waiter.waiterId} className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                            ...
                            <div className="flex-1">
                              <p className="font-medium text-gray-900">{waiter.waiterName || "Unknown Waiter"}</p>
                              <p className="text-sm text-gray-600">
                                {waiter.orders} orders • {waiter.items} items • ${waiter.averageOrderValue.toFixed(2)}{" "}
                                avg
                              </p>
                            </div>
                            <div className="text-right">
                              <p className="font-semibold text-gray-900">${waiter.amount.toFixed(2)}</p>
                              <Progress
                                value={(waiter.amount / (topWaiters[0]?.amount || 1)) * 100}
                                className="w-20 mt-1"
                              />
                            </div>
                          </div>
                    ...
                    <CardContent className="space-y-4">
                      <div className="p-4 bg-blue-50 rounded-lg">
                        ...
                        <p className="text-sm text-blue-700">
                          Your branch is performing 15% above average for this period. Keep up the excellent work!
                        </p>
                      </div>
                      <div className="p-4 bg-green-50 rounded-lg">
                        <h4 className="font-medium text-green-900 mb-2">Staff Efficiency</h4>
                        <p className="text-sm text-green-700">
                          Average order processing time has improved by 8% compared to last month.
                        </p>
                      </div>
                      <div className="p-4 bg-yellow-50 rounded-lg">
                        <h4 className="font-medium text-yellow-900 mb-2">Opportunity</h4>
                        <p className="text-sm text-yellow-700">
                          Consider cross-training staff during peak hours to optimize service delivery.
                        </p>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <TabsContent value="branch" className="space-y-4">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <ProfileItem icon={<Building2 className="w-4 h-4" />} label="Branch Name" value={branch.name} />
                      <ProfileItem
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <ProfileItem icon={<Building2 className="w-4 h-4" />} label="Manager ID" value={user.id} />
                      ...
                      />
                      <ProfileItem icon={<Award className="w-4 h-4" />} label="Role" value="Branch Manager" />
                    </CardContent>
                  </Card>
                </div>
              </TabsContent>
