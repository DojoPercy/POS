...
export default function BranchStatistics() {
  ...
  const breadcrumbs = [{ label: "Dashboard", href: "/branch" }, { label: "Statistics" }]
  ...
  const headerActions = (
    <div className="flex items-center gap-2">
      ...
      <Button variant="outline" size="sm" onClick={() => setRefresh(true)} disabled={refresh}>
        <RefreshCw className={`w-4 h-4 mr-2 ${refresh ? "animate-spin" : ""}`} />
        Refresh
      </Button>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Viewing data for: {getDateRangeLabel()}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${branch?.status === "active" ? "bg-green-500" : "bg-red-500"}`}
                    />
                    <span className="text-sm text-gray-600 capitalize">{branch?.status || "Unknown"}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              >
                <Card
                  className={`cursor-pointer transition-all duration-200 hover:shadow-md ${
                    header.name === selectedHeader.name ? "ring-2 ring-primary bg-primary/5" : "hover:bg-gray-50"
                  }`}
                ...
                >
                  <CardHeader className="pb-2">
                    <CardTitle className="flex items-center justify-between text-sm font-medium text-gray-600">
                      ...
                      <div className="p-2 bg-gray-100 rounded-lg">{header.icon}</div>
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    {isLoading || i >= headerData.length ? (
                      ...
                      <div>
                        <p className="text-2xl font-bold text-gray-900">{headerData[i]?.toString() || "0"}</p>
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <div className="h-80">
                    <ResponsiveLineChart
                      data={
                        Array.isArray(graphData[selectedHeader.graphIndex]) ? graphData[selectedHeader.graphIndex] : []
                      }
