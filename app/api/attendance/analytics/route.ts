...
export async function GET(request: NextRequest) {
  try {
    ...
    const completedShifts = attendanceRecords.filter((r) => r.signInTime && r.signOutTime).length
    const incompleteShifts = attendanceRecords.filter((r) => r.signInTime && !r.signOutTime).length
    ...
    const branchStatsWithNames = await Promise.all(
      branchStats.map(async (stat) => {
        ...
        return {
          ...
          averageHours: stat._count.id > 0 ? (stat._sum.totalHours || 0) / stat._count.id : 0,
        }
      }),
    )
    ...
    const activeSessionsWithHours = activeSessions.map((session) => {
      ...
      const currentHours = (currentTime.getTime() - signInTime.getTime()) / (1000 * 60 * 60)
