...
export async function POST(req: NextRequest){
    ...
    try {
        ...
        const {menuId , ingredientId, amount} : {menuId: string, ingredientId: string, amount: number} = body;
        ...
        console.log(error);
        return new NextResponse(`Error with Creating Ingredient: ${error}`, { status: 500 });
...
export async function DELETE(req: NextRequest){
    try {
        ...
        console.log(error);
        return new NextResponse(`Error with Deleting Ingredient: ${error}`, { status: 500 });
...
export async function PUT(req: NextRequest){
    try {
        ...
        const {id, menuId , ingredientId, amount} : {id: string, menuId: string, ingredientId: string, amount: number} = body;
        ...
        console.log(error);
        return new NextResponse(`Error with Updating Ingredient: ${error}`, { status: 500 });
