...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (let i = 0; i < days; i++) {
      ...
      const dayOrders = orders.filter((order) => order.createdAt >= dayStart && order.createdAt <= dayEnd)

      const dayItems = dayOrders.reduce(
        (sum, order) => sum + order.orderLines.reduce((lineSum, line) => lineSum + (line.quantity || 0), 0),
        0,
      )

      const daySales = dayOrders.reduce(
        (sum, order) =>
          sum + order.orderLines.reduce((lineSum, line) => lineSum + (line.quantity || 0) * (line.price || 0), 0),
        0,
      )
      ...
      dailyData.push({
        ...
        [metric]: metric === "orders" ? dayOrders.length : metric === "items" ? dayItems : Number(daySales.toFixed(2)),
      })
    }
    ...
    const result = {
      ...
      summary: {
        ...
        averageDaily: {
          ...
          items: Number((dailyData.reduce((sum, day) => sum + day.items, 0) / days).toFixed(1)),
          sales: Number((dailyData.reduce((sum, day) => sum + day.sales, 0) / days).toFixed(2)),
        },
      },
