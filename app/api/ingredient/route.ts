...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      ...
      unit,
      companyId,
    }: { name: string; unit: string; companyId: string } = body;
    ...
    NextResponse.json(
      { message: `Error with Creating Ingredient: ${error}` },
      { status: 500 },
    );
  }
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const {
      ...
      unit,
      companyId,
    }: { id: string; name: string; unit: string; companyId: string } = body;
    ...
    NextResponse.json(
      { message: `Error with Updating Ingredient: ${error}` },
      { status: 500 },
    );
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    NextResponse.json(
      { message: `Error with Deleting Ingredient: ${error}` },
      { status: 500 },
    );
  }
}
