...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!base64) {
      return new NextResponse(JSON.stringify({ error: "Base64 string is required" }), { status: 400 });
    }
    ...
    return new NextResponse(JSON.stringify({ url: result.secure_url }), { status: 200 });
  } catch (error) {
