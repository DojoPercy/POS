...
export async function GET(request: NextRequest) {
  try {
    ...
    return new NextResponse('No id, ingredientId or branchId provided', {
      status: 400,
    });
  } catch (error) {
    console.log(error);
    return new NextResponse(`Error with Getting Ingredient: ${error}`, {
      status: 500,
    });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      branchId,
      quantity,
    }: { ingredientId: string; branchId: string; quantity: number } = body;
    ...
    console.error(error);
    return new NextResponse(`Error creating/updating inventory: ${error}`, {
      status: 500,
    });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(error);
    return new NextResponse(`Error with Deleting Stock: ${error}`, {
      status: 500,
    });
  }
}
