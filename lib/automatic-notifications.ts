...
export class AutomaticNotificationService {
  ...
  static async checkTopBranches(companyId: string) {
    try {
      ...
      const response = await fetch(`/api/summary/branches?companyId=${companyId}&fromDate=${startDate.toISOString()}&toDate=${endDate.toISOString()}`)
      if (!response.ok) {
...
console.log("Top branches checked")
  ...
  static async checkHighestOrders(companyId: string) {
    try {
      ...
      const response = await fetch(`/api/orders?companyId=${companyId}&from=${startDate.toISOString()}&to=${endDate.toISOString()}`)
      if (!response.ok) {
      ...
      const averageOrderValue = orders.reduce((sum, order) => sum + order.totalPrice, 0) / orders.length
  ...
  static async checkTopAttendance(companyId: string) {
    try {
      ...
      const response = await fetch(`/api/attendance/analytics?companyId=${companyId}&startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}`)
      if (!response.ok) {
      ...
      if (!attendanceData.userStats || attendanceData.userStats.length === 0) return

      // Find employee with most hours worked
      const topHours = attendanceData.userStats.reduce((prev: any, current: any) => 
        prev.totalHours > current.totalHours ? prev : current
      )

      // Find employee with most shifts (days present)
      const topShifts = attendanceData.userStats.reduce((prev: any, current: any) => 
        prev.totalShifts > current.totalShifts ? prev : current
      )

      // Create notifications for top attendance
      if (topShifts.totalShifts >= 20) { // Threshold for perfect attendance
      ...
      if (topHours.totalHours >= 160) { // Threshold for full-time equivalent
  ...
  static async generateDailySummary(companyId: string) {
    try {
      ...
      const response = await fetch(`/api/summary/expense_revenue?companyId=${companyId}&from=${todayStr}&to=${todayStr}`)
      if (!response.ok) {
      ...
      const branchResponse = await fetch(`/api/summary/branches?companyId=${companyId}&fromDate=${todayStr}&toDate=${todayStr}`)
  ...
  static async checkLowStockAlerts(companyId: string) {
    try {
      ...
      const response = await fetch(`/api/inventory_stock?companyId=${companyId}`)
      if (!response.ok) {
    ...
    try {
      ...
      for (const branch of branches) {
        for (const milestone of milestones) {
          if (branch.revenue >= milestone && branch.revenue < milestone + 1000) {
  ...
  static async generateWeeklyAchievements(companyId: string) {
    try {
      ...
      const response = await fetch(`/api/summary/expense_revenue?companyId=${companyId}&from=${startOfWeek.toISOString()}&to=${endOfWeek.toISOString()}`)
      if (!response.ok) {
      ...
      const ordersResponse = await fetch(`/api/orders?companyId=${companyId}&from=${startOfWeek.toISOString()}&to=${endOfWeek.toISOString()}`)
      let totalOrders = 0
      ...
      const averageOrderValue = totalOrders > 0 ? weeklyData.totalRevenue / totalOrders : 0
      ...
      for (const achievement of achievements) {
        if (achievement.condition) {
          await NotificationUtils.createWeeklyAchievementNotification(achievement)
        }
      }
