...
type ExpenseSummaryResponse = {
  ...
  export async function getExpenseSumByDateRange(from: Date, to: Date, branchId?: string, companyId?: string): Promise<number > {
    const query = {
   ...
   export async function getExpensesSummaryByDateRangeOwner(
    ...
    const fromTime = Date.UTC(from.getFullYear(), from.getMonth(), from.getDate(), 0, 0, 0, 0);
    const toTime = Date.UTC(to.getFullYear(), to.getMonth(), to.getDate(), 23, 59, 59, 999);
  ...
  export async function getTotalExpenseCount(from: Date, to: Date, branchId?: string, companyId?: string): Promise< number > {
    const query = {
