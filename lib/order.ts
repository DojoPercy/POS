...
export async function getOrders(
  ...
  try {
    ...
    if (branchId) {
      const res = await fetch(`${url}?branchId=${branchId}`, {
        cache: 'no-store',
      });

      return res.json();
    } else if (waiterId) {
      const res = await fetch(`${url}?waiterId=${waiterId}`, {
        cache: 'no-store',
      });
      return res.json();
    } else {
...
export async function getOrderRevenueByDateRange(
  from: Date,
  to: Date,
  branchId?: string,
  companyId?: string,
...
export async function getOrderSummaryByDateRange(
  from: Date,
  to: Date,
  branchId?: string,
  companyId?: string,
  ...
  const summary: Record<
    string,
    { sales: number; revenue: number; income: number }
  > = {};
  let totalRevenue = 0;
  ...
  for (const order of res) {
    ...
    for (const orderLine of order.orderLines) {
      ...
      summary[orderDate].income +=
        orderLine.totalPrice - orderLine.price * orderLine.quantity;
    }
...
export async function getSalesSummaryOfBranches(
  ...
  try {
    ...
    const summary: Record<
      string,
      { branch: string; sales: number; revenue: number }
    > = {};
...
export async function getExpectedRevenueByDataRange(
  from: Date,
  to: Date,
  branchId?: string,
  companyId?: string,
